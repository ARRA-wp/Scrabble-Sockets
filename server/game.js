...
const startGame = room => {
  ...
  data[room].ready = [];
  socket.serverLog(`El juego en la sala ${room} ha comenzado!`.magenta);
  ...
  if (!data[room].options.simultaneous) {
    ...
    socket.sendGlobalAnnouncement(
      room,
      `Es el turno de ${getCurrPlayer(room)}!!`,
      'green',
    );
  }
  socket.sendUpdate(room, data[room]);
    ...
    if (
      ...
      if (data[room].status === 'playing') {
        ...
        if (
          ...
          socket.sendGlobalAnnouncement(
            room,
            `Ronda ${data[room].round} a terminado, presiona "ready" para continuar`,
            'blue',
          );
          ...
          socket.sendGlobalAnnouncement(
            ...
            } ha terminado su turno! Es el turno de ${getCurrPlayer(room)} ahora`,
            'green',
          );
          data[room].time = data[room].options.playTime;
        }
          ...
          data[room].players[i].words.forEach((word, index) => {
            if (player.words[index].challengable) {
              ...
              if (twl[player.words[index].word.toLowerCase()] === undefined) {
                socket.sendGlobalAnnouncement(
                  room,
                  `${player.name} jugaste una palabra ilegal ${
        ...
        // Game over behavior
        if (gameOverIndex >= 0) {
          data[room].status = 'Fin del Juego!';
          socket.sendGlobalAnnouncement(
            room,
            `Fin del Juego! ${data[room].players[0].name} gana con ${
          ...
          socket.sendGlobalAnnouncement(
            room,
            `Ronda ${data[room].round} empieza. Hay ${
...
const deletePlayer = (room, player) => {
  if (data[room]) {
    ...
    if (data[room].players.length === 0) {
      socket.serverLog(`Cuarto ${room} ya no existe`.red);
      delete data[room];
    }
  }
};
...
const validateBoard = (s, board, player, room) => {
  ...
  if (board[center][center].letter === '') {
    socket.sendError(s, `Se empieza en el centro!`);
    return false;
  }
    ...
    for (let col = 0; col < size; col += 1) {
      if (board[row][col].temp) {
        if (board[row][col].letter === '*') {
          socket.sendError(
            s,
            `Debes colocar fichas blancas antes de jugarlas. Picale sobre ellas para seleccionarlas!`,
          );
          return false;
        }
        if (connected.includes(row * size + col)) {
          ...
          if (!checkInLine(board, row, col)) {
            socket.sendError(s, `Las letras deben estar en linea.`);
            return false;
          }
        } else {
          socket.sendError(s, `Todas las letras deben estar conectadas.`);
          return false;
        }
      }
    }
  ...
  if (rowToUse === undefined) {
    socket.sendError(s, `Coloca almenos una letra :C`);
    return false;
  }
  ...
  for (let row = 0; row < newBoard.length; row += 1) {
    for (let col = 0; col < newBoard.length; col += 1) {
      if (newBoard[row][col].temp) {
        socket.sendError(s, `Todas las letras deben colocarse en linea.`);
        return false;
      }
  ...
  if (words.length === 0) {
    socket.sendError(s, `Coloca mas de una letra en el centro`);
    return false;
  }
  ...
  words.forEach(word => {
    ...
    socket.sendGlobalAnnouncement(
      room,
      `${player} jugo ${word.word} por ${word.points} puntos`,
      'purple',
    );
    addToPlayerData(room, player, 'puntaje', word.points);
  ...
  if (
    ...
    socket.sendGlobalAnnouncement(
      room,
      `${player} Obtubo un Bingo! +50 puntos!`,
      'purple',
    );
    addToPlayerData(room, player, 'puntaje', 50);
  }
...
const setReady = (room, player) => {
  if (!data[room].ready.includes(player)) {
    data[room].ready.push(player);
    socket.sendGlobalAnnouncement(room, `${player} esta listo!`, 'green');
  }
};
...
const challenge = (room, you, them) => {
  ...
  getPlayerData(room, them).words.map(word => {
    if (word.challengable) {
      ...
      if (
        ...
        addToPlayerData(room, them, 'Puntaje', -word.points);
        socket.sendGlobalAnnouncement(
          room,
          `${word.word} No es una palabra valida!`,
          'blue',
        );
        newWord.points = 0;
  ...
  if (invalidWords.length > 0) {
    socket.sendGlobalAnnouncement(room, `${them} Pierde un turno.`, 'blue');
    // for (let row = 0; row < data[room].board.length; row += 1) {
    //   for (let col = 0; col < data[room].board.length; col += 1) {
    //     if (
    ...
    socket.sendGlobalAnnouncement(
      room,
      `Todas las palabras jugadas por ${them} son validas! ${you} pierdes un turno.`,
      'blue',
    );
  }
  socket.sendUpdate(room, data[room]);
